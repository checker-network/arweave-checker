...
const fetchData = async () => {
    ...
    for (let randomTx of randomsTxs) {
        ...
        for(let arNode of arweaveNodes) {
            ...
            try {
                ...
                output = [...output, { txId, node: arNode,  result: { status: "success", data: tx }}]
            } catch (err) {
                output = [...output, { txId, node: arNode,  result: { status: "error", data: err }}]
            }
        }
    }
    ...
    fs.writeFile('./src/data/arweave-retrieve-transaction-output.json', jsonString, (err) => {
        if (err) {
